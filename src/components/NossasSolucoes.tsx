...
const NossasSolucoes = () => {
  const solucoes = [
    ...
    { 
      title: "Design UX / UI", 
      category: "Interface",
      desc: "Experiências memoráveis e interfaces intuitivas que conectam sua marca aos usuários.", 
      icon: (
        <svg className="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div className="container mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5">
            {solucoes.map((item, i) => (
              <div 
                key={i} 
                className="bg-white p-6 rounded-[28px] shadow-sm hover:shadow-xl hover:bg-qwa-lilac transition-all duration-500 flex flex-col group cursor-pointer border border-white/10"
              >
                <div className="flex items-center gap-3 mb-5">
                  <div className="w-10 h-10 flex items-center justify-center text-qwa-blue group-hover:text-white transition-colors duration-300">
                    {item.icon}
                  </div>
                  <span className="text-[10px] font-bold text-gray-400 uppercase tracking-widest group-hover:text-white/60 transition-colors">
                    {item.category}
                  </span>
                </div>

                <h3 className="text-lg font-black text-qwa-dark mb-3 leading-tight group-hover:text-white transition-colors">
                  {item.title}
                </h3>
                
                <p className="text-gray-400 text-xs font-medium leading-relaxed mb-6 line-clamp-3 group-hover:text-white/70 transition-colors">
                  {item.desc}
                </p>

                <div className="mt-auto flex items-center justify-end">
                  <div className="w-10 h-10 rounded-2xl bg-qwa-dark text-white flex items-center justify-center opacity-0 group-hover:opacity-100 group-hover:bg-white group-hover:text-qwa-dark transition-all duration-500 translate-x-4 group-hover:translate-x-0">
